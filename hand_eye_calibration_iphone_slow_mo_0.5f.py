import cv2
import numpy as np
import glob
import os
import random
import sys
import matplotlib.pyplot as plt

# ================== 用户输入参数 ==================
checker_width = 0.015     # 每个小方格的宽度 (15mm)
board_rows = 6
board_cols = 8
image_dir = "/home/xxz/code/toss-reorient/RGBTrack/demo_data/calib_images/selected_rgb"
num_samples = 40
output_dir = "/home/xxz/code/toss-reorient/RGBTrack/demo_data/calib_images"
# ==================================================

pattern_size = (board_cols - 1, board_rows - 1)

# 构建棋盘格三维点
objp = np.zeros((1, pattern_size[0] * pattern_size[1], 3), np.float32)
objp[0, :, :2] = np.mgrid[0:pattern_size[0], 0:pattern_size[1]].T.reshape(-1, 2)
objp *= checker_width

objpoints, imgpoints = [], []
img_size = None

images = sorted(glob.glob(os.path.join(image_dir, "*.jpg")) +
                glob.glob(os.path.join(image_dir, "*.png")))
total_images = len(images)
print(f"Total images found: {total_images}")

if total_images > num_samples:
    T = total_images // num_samples
    images = [images[i] for i in range(0, total_images, T)][:num_samples]
else:
    T = 1  # 不足 num_samples 时全部使用
print(f"Sampling every {T} image(s): total {len(images)} images for fisheye calibration...\n")


# === 检测角点 ===
for fname in images:
    img = cv2.imread(fname)
    if img is None:
        continue
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    img_size = gray.shape[::-1]
    ret, corners = cv2.findChessboardCorners(
        gray, pattern_size,
        cv2.CALIB_CB_ADAPTIVE_THRESH + cv2.CALIB_CB_NORMALIZE_IMAGE
    )
    if ret:
        corners2 = cv2.cornerSubPix(
            gray, corners, (11, 11), (-1, -1),
            (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 100, 1e-6)
        )
        objpoints.append(objp)
        imgpoints.append(corners2)
        print(f"{os.path.basename(fname)}: corners detected")
    else:
        print(f"{os.path.basename(fname)}: no corners detected")

print(f"\nValid frames used: {len(objpoints)}")
if len(objpoints) < 5:
    raise RuntimeError("Not enough valid images for fisheye calibration!")

# === 初次标定 ===
print("\nRunning initial fisheye calibration...")
K = np.zeros((3, 3))
D = np.zeros((4, 1))
rvecs, tvecs = [], []
flags = (cv2.fisheye.CALIB_RECOMPUTE_EXTRINSIC |
         cv2.fisheye.CALIB_FIX_SKEW |
         cv2.fisheye.CALIB_CHECK_COND)

rms, K, D, rvecs, tvecs = cv2.fisheye.calibrate(
    objpoints, imgpoints, img_size, K, D, rvecs, tvecs,
    flags=flags,
    criteria=(cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 200, 1e-7)
)

print(f"\nInitial RMS: {rms:.5f}")
print("K:\n", K)
print("D:\n", D.ravel())

# === 计算每张图片的重投影误差 ===
errors = []
for i in range(len(objpoints)):
    try:
        pts, _ = cv2.fisheye.projectPoints(objpoints[i], rvecs[i], tvecs[i], K, D)
        pts = pts.reshape(-1, 1, 2)  # ✅ 修复维度不匹配
        if pts.shape != imgpoints[i].shape:
            print(f"[Warning] Shape mismatch at frame {i}: pts={pts.shape}, imgpoints={imgpoints[i].shape}")
            continue
        err = cv2.norm(imgpoints[i], pts, cv2.NORM_L2) / len(pts)
        errors.append(err)
    except Exception as e:
        print(f"[Warning] Skipping frame {i} for error calculation: {e}")

if len(errors) == 0:
    raise RuntimeError("No valid reprojection errors computed — check chessboard detection or image consistency.")

mean_err = np.mean(errors)
std_err = np.std(errors)
thresh = mean_err + 2 * std_err

print(f"\nMean per-image error: {mean_err:.4f}, Std: {std_err:.4f}, Threshold: {thresh:.4f}")
good_indices = [i for i, e in enumerate(errors) if e < thresh]
print(f"Removing {len(errors) - len(good_indices)} outlier(s)...")

# === 剔除异常帧后重新标定 ===
objpoints = [objpoints[i] for i in good_indices]
imgpoints = [imgpoints[i] for i in good_indices]

# ✅ 重新初始化向量以避免 channels 错误
rvecs, tvecs = [], []

rms_refined, K, D, rvecs, tvecs = cv2.fisheye.calibrate(
    objpoints, imgpoints, img_size, K, D, rvecs, tvecs,
    flags=flags,
    criteria=(cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 200, 1e-7)
)

print("\n=== Refined Fisheye Calibration Results ===")
print("RMS Reprojection Error:", rms_refined)
print("Camera Matrix (K):\n", K)
print("Distortion Coefficients (k1,k2,k3,k4):\n", D.ravel())

# === 保存参数 ===
np.savez(os.path.join(output_dir, "iphone_checkerboard_fisheye_refined.npz"),
         K=K, D=D, rms=rms_refined)
print(f"\nSaved refined parameters to iphone_checkerboard_fisheye_refined.npz")

# === 绘制误差图 ===
plt.figure(figsize=(10, 4))
plt.bar(range(len(errors)), errors, color='gray', label='All Images')
plt.axhline(mean_err, color='r', linestyle='--', label='Mean Error')
plt.axhline(thresh, color='orange', linestyle='--', label='Threshold')
plt.xlabel("Image Index")
plt.ylabel("Reprojection Error (px)")
plt.title("Fisheye Per-Image Reprojection Errors")
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "fisheye_error_distribution.png"))
print(f"Saved error distribution to fisheye_error_distribution.png")

# === 去畸变示例 ===
test_img = cv2.imread(images[0])
map1, map2 = cv2.fisheye.initUndistortRectifyMap(
    K, D, np.eye(3), K, img_size, cv2.CV_16SC2
)
undistorted = cv2.remap(test_img, map1, map2, interpolation=cv2.INTER_LINEAR)

h, w = test_img.shape[:2]
combined = np.hstack((test_img, undistorted))
cv2.putText(combined, "Original", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 2)
cv2.putText(combined, "Fisheye Undistorted", (w + 50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 2)
output_path = os.path.join(output_dir, "calibration_result_fisheye_refined.png")
cv2.imwrite(output_path, combined)
print(f"Saved comparison image to: {output_path}")

sys.exit(0)
