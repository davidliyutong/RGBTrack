    ...
    def _apply_undistortion(self, frame: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            dist_coeffs = np.array([
                self.config.camera.distortion_k1,
                self.config.camera.distortion_k2,
                self.config.camera.distortion_p1,
                self.config.camera.distortion_p2,
                self.config.camera.distortion_k3
            ], dtype=np.float32)
