...
class WebUI:
    ...
    def __init__(
        ...
        Args:
            ...
            detection_config: Initial detection configuration
            ...
            on_detection_config_update: Callback when detection configuration is updated
        ...
        with gr.Blocks(title="RGBTrack Camera Control") as app:
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    with gr.Row():
                        ...
                        wb_calibrate_btn = gr.Button("Calibrate WB", variant="secondary")

                    gr.Markdown("#### Manual RGB Balance (Only active when white balance mode is set to 'manual')")
                    ...
                    gr.Markdown("### Distortion Parameters (k1, k2, p1, p2, k3)")
                    ...
                    undistort_checkbox = gr.Checkbox(
                        ...
                        info="Apply cv2.undistort to captured frames"
                    )
                ...
                with gr.Column(scale=2):
                    ...
                    with gr.Row():
                        ...
                        with gr.Column():
                            distortion_toggle = gr.Checkbox(
                                label="Enable Distortion Correction",
                                value=False,
                                info="Apply distortion correction to preview"
                            )
                    ...
                    detection_prompt = gr.Textbox(
                        ...
                        info="Prompt for Segment Anything Model (SAM)",
                        lines=2
                    )
                    ...
                    mesh_path_text = gr.Textbox(
                        ...
                        info="Path to detection model file"
                    )

                    # Apply detection config button
                    apply_detection_btn = gr.Button("Apply Detection Config", variant="primary")
                    detection_status = gr.Textbox(label="Detection Config Status", interactive=False)
            ...
            def update_detection_config(prompt, confidence, nms, mesh_path):
                ...
                try:
                    ...
                    return f"✓ Detection configuration updated and saved to {self.config_path}"
                except Exception as e:
            ...
            def calibrate_wb():
                ...
                try:
                    ...
                    # Update UI with calibrated values from config
                    status = f"✓ White balance calibrated: R={self.config.red_balance:.2f}, G={self.config.green_balance:.2f}, B={self.config.blue_balance:.2f}"
                    return (
                        status,
                    ...
                    # Return error status and keep current values
                    return (
                        f"✗ Error: {str(e)}",
                        self.config.red_balance,
                        self.config.green_balance,
            ...
            apply_detection_btn.click(
                fn=update_detection_config,
                inputs=[
                    detection_prompt,
                    confidence_slider,
                    nms_slider,
            ...
            preview_btn.click(
                fn=take_preview,
                inputs=[detection_toggle, distortion_toggle],
